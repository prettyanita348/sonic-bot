...
function setupCronJob(
  ...
  console.log(colors.Yellow('Setting up cron job to run every 24 hours...'));
...
(async () => {
  ...
  // Ask if user wants to use predefined addresses or generate new ones
  const addressChoice = readlineSync.question(
    'Do you want to use predefined addresses or generate random addresses? (0 for predefined, 1 for random): '
  );

  let addresses;
  if (addressChoice === '0') {
    // Load predefined addresses from file
    addresses = JSON.parse(fs.readFileSync('predefinedAddresses.json', 'utf-8'));
    if (!Array.isArray(addresses) || addresses.length === 0) {
      throw new Error(
        colors.red('predefinedAddresses.json is not set correctly or is empty')
      );
    }
  } else if (addressChoice === '1') {
    ...
    addresses = generateRandomAddresses(addressCount);
  } else {
    throw new Error(colors.red('Invalid selection for address method'));
  }
  ...
  if (executionMode === '0') {
    ...
    setupCronJob(seedPhrasesOrKeys, addresses, amountToSend, delayBetweenTx);
  } else {
